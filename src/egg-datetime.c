 ...
 * $Id: egg-datetime.c 558 2005-06-16 13:27:40Z tml $
 *
...
static void	 cal_popup_today_button_clicked (EggDateTime *edt, GtkWidget *button);
...
/*	gtk_box_set_spacing (GTK_BOX (edt), 4);*/
	...
	gtk_box_pack_start (GTK_BOX (edt), priv->date_box, FALSE, TRUE, 0);
	...
	gtk_frame_set_shadow_type(GTK_FRAME(box),GTK_SHADOW_ETCHED_IN);	
	gtk_container_add(GTK_CONTAINER (priv->cal_popup), box);
	gtk_widget_show(box);
	...
	gtk_box_pack_start(GTK_BOX(vbox), priv->calendar, FALSE, TRUE, 0);
	...
	/* Button today */
	priv->today_button = gtk_button_new_with_mnemonic(_("Today"));	
	gtk_box_pack_start(GTK_BOX(vbox), priv->today_button, FALSE, TRUE, 0);
	
	g_signal_connect_swapped(G_OBJECT(priv->today_button), "clicked", 
				 G_CALLBACK(cal_popup_today_button_clicked), edt);
...
static void
cal_popup_today_button_clicked (EggDateTime *edt, GtkWidget *button)
{
 ...
 **/
GtkWidget *
egg_datetime_new_from_datetime (GDateYear year, GDateMonth month, GDateDay day)
{
	EggDateTime *edt;
