...
static GObject * stuffkeeper_data_boolean_config_constructor (GType type, guint n_construct_properties, GObjectConstructParam * construct_properties) {
	...
	self = STUFFKEEPER_DATA_BOOLEAN_CONFIG (obj);
	{
		...
		label = g_object_ref_sink (((GtkLabel*) (gtk_label_new ("Default value"))));
		_tmp0 = NULL;
		self->priv->checkbox = (_tmp0 = g_object_ref_sink (((GtkCheckButton*) (gtk_check_button_new ()))), (self->priv->checkbox == NULL ? NULL : (self->priv->checkbox = (g_object_unref (self->priv->checkbox), NULL))), _tmp0);
...
GType stuffkeeper_data_boolean_config_get_type (void) {
	static volatile gsize stuffkeeper_data_boolean_config_type_id = 0;
	if (g_once_init_enter (&stuffkeeper_data_boolean_config_type_id)) {
		GType stuffkeeper_data_boolean_config_type_id_temp;
		...
		stuffkeeper_data_boolean_config_type_id_temp = g_type_register_static (GTK_TYPE_HBOX, "StuffkeeperDataBooleanConfig", &g_define_type_info, 0);
		g_once_init_leave (&stuffkeeper_data_boolean_config_type_id, stuffkeeper_data_boolean_config_type_id_temp);
	}
